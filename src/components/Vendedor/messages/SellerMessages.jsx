  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">No hay mensajes aún</p>
            <p className="text-[10px] text-slate-400 px-10 mt-2 italic">Aclara dudas al cliente sobre su envío o pago.</p>
          </div>
        ) : (
          messages.map((m) => (
            <div key={m._id} className={`flex ${m.senderType === "seller" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] p-3 rounded-2xl shadow-sm ${
                ...
                <div className={`flex items-center gap-1 mt-1 text-[9px] opacity-70 ${m.senderType === "seller" ? "justify-end" : "justify-start"}`}>
                  <Clock size={10} />
      ...
      <div className="p-4 bg-white border-t">
        <form onSubmit={sendMessage} className="flex gap-2 bg-slate-100 p-2 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-emerald-500 transition-all">
          <input
            value={text}
          ...
          <button 
            type="submit" 
            disabled={!text.trim()}
            className="bg-emerald-600 p-2.5 rounded-xl text-white hover:bg-emerald-700 disabled:opacity-50 transition-all shadow-lg shadow-emerald-100"
          >
            <Send size={18} />
          </button>
        </form>
        <p className="text-[9px] text-slate-400 text-center mt-2 font-bold uppercase tracking-tight">
