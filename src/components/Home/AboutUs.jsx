...
export default function AboutUs() {
  ...
  // Verificamos si hay una sesión activa para ocultar secciones de venta
  const token = localStorage.getItem("token");

  // Función de retorno inteligente
  const handleBack = () => {
    if (token) {
      // Si está logueado como usuario, vuelve a su Home de comprador
      navigate("/homeUser");
    } else {
      // Si es visitante, vuelve a la Landing Page
      navigate("/");
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-16 font-sans text-gray-800 relative">
      ...
      {/* CTA FINAL CONDICIONAL: Solo aparece si NO hay un comprador logueado */}
      {!token ? (
        <section className="text-center bg-gray-50 rounded-[3rem] p-12 md:p-20 border border-gray-100 mb-16 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        /* Pie de página sutil para usuarios logueados */
        <div className="text-center py-12 border-t border-gray-100 italic text-gray-400 text-xs uppercase tracking-widest">
...
// Sub-componentes se mantienen iguales
function StatCard({ value, label, mt = "" }) {
  return (
    <div className={`bg-white/5 backdrop-blur-md p-4 md:p-6 rounded-[2.5rem] border border-white/10 text-center hover:bg-white/10 transition-all flex flex-col justify-center items-center min-h-[120px] md:min-h-[160px] ${mt}`}>
      ...
      <p className="text-[8px] md:text-[10px] uppercase tracking-widest font-black text-gray-500 italic leading-tight">{label}</p>
    </div>
  );
}
...
function ValueCard({ icon, title, text }) {
  return (
    <div className="bg-white p-10 md:p-12 rounded-[3rem] shadow-xl shadow-gray-200/40 border border-transparent hover:border-blue-100 transition-all duration-300 text-center group">
